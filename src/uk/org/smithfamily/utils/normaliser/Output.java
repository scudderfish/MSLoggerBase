...
public class Output
{
	...
	private static Set<String> alwaysDouble = new HashSet<String>(Arrays.asList(new String[] { "pulseWidth", "throttle", "accDecEnrich", "accDecEnrichPcnt", "accEnrichPcnt",
			"accEnrichMS", "decEnrichPcnt", "decEnrichMS", "time", "egoVoltage", "egoVoltage2", "egoCorrection", "veCurr", "lambda", "TargetLambda" }));
	...
	/**
	 * This is nasty. We need to have a set of methods to init the constants as
	 * there is a hard limit of 64K on the size of a method, and MS3 will break
	 * that. We also need to ensure that if there is any preprocessor type logic
	...
	static void outputLoadConstants(ECUData ecuData, PrintWriter writer)
	{
		...
		for (Constant c : ecuData.getConstants())
		{
			...
			if (!"PREPROC".equals(c.getType()))
			{
				...
				{
					def = getScalar("pageBuffer", ecuData.getConstantVars().get(name), name, c.getType(), "" + c.getOffset(), "" + c.getScale(), "" + c.getTranslate());
				}
	...
	private static String generateLoadArray(ECUData ecuData, Constant c)
	{
		...
		{
			functionName += "Vector";
			loadArray = String.format("%s = %s(pageBuffer, %d, %d, %s, %s, %s, %s);", c.getName(), functionName, c.getOffset(), width, c.getScale(), c.getTranslate(),
					c.getDigits(), signed);

		} else
		{
			functionName += "Array";
			loadArray = String.format("%s = %s(pageBuffer, %d, %d, %d, %s, %s, %s, %s);", c.getName(), functionName, c.getOffset(), width, height, c.getScale(), c.getTranslate(),
					c.getDigits(), signed);
		}
		return loadArray;
	}
