...
public class MultiTouchController<T> {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        try {
            @SuppressWarnings("ConstantConditions") int pointerCount = multiTouchSupported ? (Integer) m_getPointerCount.invoke(event) : 1;
            if (DEBUG)
    ...
    private void decodeTouchEvent(int pointerCount, int action, boolean down, long eventTime) {
        if (DEBUG)
        ...
        mCurrPt.set(pointerCount, action, down, eventTime);
    ...
    public static class PointInfo {
        ...
        /** Set all point info */
        private void set(int numPoints, int action, boolean isDown, long eventTime) {
            if (DEBUG)
                Log.i("MultiTouch", "Got here 8 - " + numPoints + " " + MultiTouchController.xVals[0] + " " + MultiTouchController.yVals[0] + " " + (numPoints > 1 ? MultiTouchController.xVals[1] : MultiTouchController.xVals[0]) + " "
                        + (numPoints > 1 ? MultiTouchController.yVals[1] : MultiTouchController.yVals[0]) + " " + action + " " + isDown);
            this.eventTime = eventTime;
            this.action = action;
            this.numPoints = numPoints;
            for (int i = 0; i < numPoints; i++) {
                this.xs[i] = MultiTouchController.xVals[i];
                this.ys[i] = MultiTouchController.yVals[i];
                this.pressures[i] = MultiTouchController.pressureVals[i];
                this.pointerIds[i] = MultiTouchController.pointerIds[i];
            }
            this.isDown = isDown;
            this.isMultiTouch = numPoints >= 2;

            if (isMultiTouch) {
                xMid = (MultiTouchController.xVals[0] + MultiTouchController.xVals[1]) * .5f;
                yMid = (MultiTouchController.yVals[0] + MultiTouchController.yVals[1]) * .5f;
                pressureMid = (MultiTouchController.pressureVals[0] + MultiTouchController.pressureVals[1]) * .5f;
                dx = Math.abs(MultiTouchController.xVals[1] - MultiTouchController.xVals[0]);
                dy = Math.abs(MultiTouchController.yVals[1] - MultiTouchController.yVals[0]);

            } else {
                // Single-touch event
                xMid = MultiTouchController.xVals[0];
                yMid = MultiTouchController.yVals[0];
                pressureMid = MultiTouchController.pressureVals[0];
                dx = dy = 0.0f;
            }
